...
The block is a pair of beamed semiquavers, and nothing else. The note is a formula
-- the constants below are the whole of it -- and this script turns the formula
into the shipped geometry. Adjust and re-run.

It is baked into the mesh rather than added at load for two reasons: Besiege draws
the toolbar icon from the block's mesh, so anything a behaviour adds at runtime is
missing from the icon; and geometry needs no material of its own, so there is no
shader to find and nothing to fail at load. Its colour comes from a single texel of
the texture below, which is what lets the block light it -- see the block's
`Dress` and `Ink`.
"""
...
# How tall the note stands in mesh units. The mesh is worn at half scale, so this
# is 0.9 of a block: the note is the whole block now, not an ornament inside a cage.
NOTE_HEIGHT = 1.8
...
def main():
    out = ["# Braids Synth block. Generated by tools/make-block-mesh.py -- edit that,",
           "# not this. A pair of beamed semiquavers, from the constants in that file.",
           "o SynthBlock"]

    verts, uvs, normals, faces = [], ["vt %.6f %.6f" % INK_UV], [], []
    for tri, n in build_note():
        ...
        faces.append(" ".join("%d/1/%d" % (i, ni) for i in idx))

    out += verts + uvs + normals + ["s off"] + ["f " + f for f in faces]
    io.open(MESH, "w").write("\n".join(out) + "\n")
    print("mesh: %d verts, %d faces, all note" % (len(verts), len(faces)))
