...
#
# The folder Besiege loads is BraidsSynth/, not the repository root: that
...
if [[ ! -f "$SRC/BraidsSynth.dll" ]]; then
    echo "BraidsSynth/BraidsSynth.dll is missing; run ./tools/build.sh first." >&2
    exit 1
fi
